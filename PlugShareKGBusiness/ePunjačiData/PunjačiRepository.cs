...
{
    public class PunjačiRepository : IPunjačiRepository
    {
        public List<Punjač> SviPunjači()
        {
            List<Punjač> listaPunjača = new List<Punjač>();

            string query = $"select p.*, b.* from punjači p join bazne_stanice b on p.Id_bazne_stanice=b.Id_stanice";
            using (SqlCommand sqlCommand = BaseConnection.GetSqlCommand(query))
            {
                ...
                if (dataReader.HasRows)
                {
                    while (dataReader.Read())
                    {
                        ...
                        punjač.Snaga = dataReader.GetInt32(2);
                        punjač.Id_bazne_stanice = dataReader.GetString(1);
        ...
        {
            string query = string.Format($"insert into punjači(Id_punj, Snaga, Id_bazne_stanice) values('{punjač.Id_punj}', '{punjač.Snaga}', '{punjač.Id_bazne_stanice}')");
