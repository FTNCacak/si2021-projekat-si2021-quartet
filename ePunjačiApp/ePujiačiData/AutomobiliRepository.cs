...
{
    public class AutomobiliRepository : IAutomobiliRepository
    {
        public List<Automobil> SviAutomobili()
        {
            List<Automobil> listaAutomobila = new List<Automobil>();

            string query = $"select a.*, k.*, pr.* from automobili a inner join korisnici k on  k.JMBG = a.JMBG_korisnika inner join priključci pr on pr.Id_prik = a.Id_priključka";
            using (SqlCommand sqlCommand = BaseConnection.GetSqlCommand(query))
            {
                ...
                if (dataReader.HasRows)
                {
                    while (dataReader.Read())
                    {
                        ...
                        automobil.JMBG_korisnika = dataReader.GetInt64(2);
                        ...
                        korisnik.JMBG = dataReader.GetInt64(6);
                        ...
                        priključak.Id_punjača = dataReader.GetString(12);
        ...
        {
            string query = string.Format($"update automobili set Marka = '{automobil.Marka}', Tip = '{automobil.Tip}', Kapacitet_baterije = '{automobil.Kapacitet_baterije}' where Broj_Šasije = '{automobil.Broj_šasije}' and Id_priključka = '{automobil.Id_priključka}' and JMBG_korisnika = '{automobil.JMBG_korisnika}'");
            return BaseConnection.ExecuteNonQuerySqlCommand(query);
        }
