...
namespace ePunjačiData
{
    public class KorisniciRepository : IKorisniciRepository
    {
        ...
        {
            string query = $"update korisnici set Ime = '{korisnik.Ime}', Prezime = '{korisnik.Prezime}', Broj_telefona = '{korisnik.Broj_telefona}', Email = '{korisnik.Email}' where JMBG = '{korisnik.JMBG}'";
            return BaseConnection.ExecuteNonQuerySqlCommand(query);
        }
        public bool ObrišiKorisnik(long jmbg)
